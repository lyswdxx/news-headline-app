// ==========================================
// 热点聚合接口：Google + 天行 + VVHAN 三方合并（增强清理版）
// ==========================================
...
async function fetchTianApi(apiPath, num = 15) {
  ...
  try {
    ...
    return data.newslist.map(item => ({
      ...
      summary: cleanText(item.description || item.content || '').slice(0, 200),
...
async function aggregateNews({ topic, kw, limit = 6 }) {
  ...
  for (const r of results) {
    if (r.status === 'fulfilled' && r.value.length) allNews.push(...r.value);
  }
  const unique = [];
  const seen = new Set();
